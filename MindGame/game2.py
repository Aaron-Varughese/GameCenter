...
def draw_menu():
    ...
    rps_text = font_small.render("2. Stone Paper Scissors", True,
    BLACK)
...
def memory_booster_game():
    ...
    while running:
        ...
        for event in pygame.event.get():
            ...
            if event.type == pygame.KEYDOWN and event.key == pygame.K_q:  # Quit option
                return_to_menu()
...
def stone_paper_scissors_game():
    ...
    while running:
        ...
        if game_over:
            ...
            draw_text("Press Q to go back to menu!", 120, 250)
        else:
...
def tic_tac_toe_game():
    ...
    def draw_x(col, row):
        ...
        pygame.draw.line(screen, RED, (col * CELL_SIZE + 50, row * CELL_SIZE + CELL_SIZE - 50),
                         (col * CELL_SIZE + CELL_SIZE - 50, row * CELL_SIZE + 50), LINE_WIDTH)
    ...
    while True:
        ...
        if game_over:
            winner_text = font.render(f"{check_winner(board)} wins!", True, BLACK)
...
def return_to_menu():
    ...
    draw_menu()
    pygame.display.update()
    time.sleep(2)  # Show the menu for a brief moment
